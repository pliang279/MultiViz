...
class MOSEIDataset:
    def __init__(self, split="test", path="/home/paul/MultiBench/mosei_senti_data.pkl"):
        ...
        self.dataset = get_simple_processed_data(path)[theindex]
...
# ========================================================================================
# Most of the stuff below are copied over from MultiBench and are mostly useless
...
class Affectdataset(Dataset):
    ...
    def __getitem__(self, indd):
        ...
        label = (
            torch.tensor(get_class(self.dataset["labels"][ind])).long()
            if self.task == "classification"
            else torch.tensor(self.dataset["labels"][ind]).float()
        )
...
def get_dataloader(
    ...
    if no_robust:
        test = DataLoader(
            Affectdataset(processed_dataset["test"], flatten_time_series, task=task),
            shuffle=False,
            num_workers=num_workers,
            batch_size=batch_size,
            collate_fn=process,
    ...
    if robust_test:
        ...
        for i in range(10):
            ...
            test["vision"] = timeseries_robustness(
                [alldata["test"]["vision"].copy()], noise_level=i / 10, rand_drop=False
            )[0]
        ...
        for i in range(10):
            ...
            test["audio"] = timeseries_robustness(
                [alldata["test"]["audio"].copy()], noise_level=i / 10, rand_drop=False
            )[0]
            test["text"] = alldata["test"]["text"].copy()
            test["labels"] = alldata["test"]["labels"]
            test = drop_entry(test)
        ...
        for i in range(10):
            robust_timeseries_tmp = timeseries_robustness(
                [
                    alldata["test"]["vision"].copy(),
                    alldata["test"]["audio"].copy(),
...
def get_simple_processed_data(path, raw_path=None):
    if raw_path is None:
        ...
        trains, valids, tests = get_dataloader(
            path, robust_test=False, raw_path=raw_path
        )
    return (
        simple_process(trains, True),
        simple_process(valids, False),
        simple_process(tests["all"][0], False),
    )

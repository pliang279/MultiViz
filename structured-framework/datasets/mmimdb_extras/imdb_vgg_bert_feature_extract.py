...
tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', model_max_length=512, truncation_size='right')
...
def main():
    ...
    for i in tqdm(range(n_samples)):
        ...
        words = data['plot']
        encoded_input = tokenizer(words, return_tensors='pt', truncation=True).to(device)
        with torch.no_grad():

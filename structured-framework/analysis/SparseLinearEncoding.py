...
def getembeds(datainstances, analysismodel, reallabel=True):
    ...
    for resultobj in analysismodel.forwardbatch(datainstances):
        if reallabel:
            embeds.append(
                (
                    analysismodel.getprelinear(resultobj),
                    analysismodel.getcorrectlabel(datainstances[ind]),
                    ind,
                )
            )
        else:
            embeds.append(
                (
                    analysismodel.getprelinear(resultobj),
                    analysismodel.getpredlabel(resultobj),
                    ind,
                )
            )
        ind += 1
...
def get_sparse_linear_model(
    ...
    for model_idx in tqdm(model_idxs):
        ...
        linear.eval()
        accuracy, sparse_cnt, numel = evaluate_sparse_linear_model(
            test_loader, linear, analysismodel.device
        )

...
analysismodel = IMDb_LF(
    model_path, multibench_path, word2vec, vgg16_model, device, batch_size=32
)
...
for i in np.random.randint(low=0, high=datas.length(), size=10):
    ...
    visualizelime(
        ...
        "image",
        predlabel,
        f"visuals/imdb/imdb-lf-{i}-image-lime-pred.png",
    )
    visualizelime(
        explanation1,
        "image",
        correctlabel,
        f"visuals/imdb/imdb-lf-{i}-image-lime-correct.png",
    )
    visualizelime(
        explanation2, "text", predlabel, f"visuals/imdb/imdb-lf-{i}-text-lime-pred.png"
    )
    visualizelime(
        explanation2,
        "text",
        correctlabel,
        f"visuals/imdb/imdb-lf-{i}-text-lime-correct.png",
    )

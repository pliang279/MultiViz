...
for instance_idx, tid_dict in id_to_tids.items():
    ...
    for key, value in tid_dict.items():
        ...
        key_to_logits[instance_idx][key]['original_logits'] = original_logits
        ...
        normalized_grads = normalize255(torch.sum(torch.abs(grads), dim=0), fac=255).detach().cpu().numpy()
        img = cv2.resize(np.asarray(Image.open(instance[0])), (normalized_grads.shape[1], normalized_grads.shape[0]))
